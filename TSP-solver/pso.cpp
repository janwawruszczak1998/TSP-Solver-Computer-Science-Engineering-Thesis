...
void PSO::calculate_solution() {
    for(unsigned i = 0; i < graph.get_order() * graph.get_order(); ++i){
        swarm.push_back(Particle(0.1, 0.2, 0.7));
    ...
    while(run_flag){
        for(auto& particle : swarm){
            ...
            std::cout << "prev" << particle.particle_solution[0] << " "<< particle.particle_solution[1] << " "<< particle.particle_solution[2] << " "<< particle.particle_solution[3] << std::endl;
            particle.move_to_next_solution();
            std::cout << "next" << particle.particle_solution[0] << " "<< particle.particle_solution[1] << " "<< particle.particle_solution[2] << " "<< particle.particle_solution[3] << std::endl;
...
void PSO::Particle::calculate_new_velocity(){
    ...
    for(unsigned i = 0; i < particle_solution.size(); ++i){
        unsigned vertex = particle_solution[i];
        unsigned found_index = particle_solution.size();
...
PSO::Particle::Velocity& PSO::Particle::Velocity::operator*=(const double rhs){
    double d = rhs;
    std::vector<unsigned> from_(from);
    std::vector<unsigned> to_(to);
    unsigned size_{size};
    from.clear();
    to.clear();
    size = 0;
